...
const app = {
    ...
    setupEventListeners() {
        ...
        if (osForm) {
            osForm.addEventListener('submit', (e) => {
                // Prevenir envio padrão - vamos usar nosso método
                e.preventDefault();
    ...
    // 🆕 FUNÇÃO PRINCIPAL DE ENVIO DE OS COM E-MAIL
    abrirOSComEmail(event) {
        ...
        // Configurar assunto do e-mail conforme especificado
        const assunto = `[NOVA O.S] - ${setor} - ${funcionario}`;
        
        // Atualizar campo de assunto no formulário
    ...
    // 🆕 FUNÇÃO PARA VOLTAR AO FORMULÁRIO DE OS
    voltarParaFormOS() {
        // Mostrar formulário
        ...
        if (this.currentUser) {
            document.getElementById('os-funcionario').value = this.currentUser.nome;
            document.getElementById('os-email').value = `${this.currentUser.user}@porter.com`;
        }
    },
    ...
    registrarEnvioDetalhadoOS(osData) {
        ...
        const registro = {
            ...
            destinatarios: ['londrina.operacional@porter.com.br'],
            condo: osData.condo,
...
========================================`;
    ...
    loadChat() {
        ...
        // FIX: Scroll suave para a última mensagem
        setTimeout(() => {
    ...
    // FIX: chat individual - carregar usuários para chat privado
    ...
    // FIX: chat individual - carregar chat privado
    loadPrivateChat() {
    ...
    // FIX: chat individual - enviar mensagem privada corrigido
    ...
    criarNotificacaoChatPrivado(chatMessage) {
        ...
        const destinatario = DATA.funcionarios.find(f => f.user === chatMessage.receiver);
